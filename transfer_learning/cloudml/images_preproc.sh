...
# Usage of the script: from the folder where this script lives, run in the command line:
# ./images_preproc.sh gs://your-bucket-name
# For example:
# ./images_preproc.sh gs://wellio-kadaif-tasty-images-project-images
...
declare -r JOB_ID="food_${USER}_$(date +%Y%m%d_%H%M%S)"
declare -r DFJOB_ID="food-$(date +%Y%m%d-%H%M%S)"
declare -r BUCKET=$1
declare -r GCS_PATH="${BUCKET}/${USER}/${JOB_ID}"
declare -r DICT_FILE=gs://wellio-kadaif-tasty-images-project-images/images_dictionary.txt

echo
echo "Using job id: " $JOB_ID
echo "Using GCS_PATH: " $GCS_PATH
echo "Using output path: " "${GCS_PATH}/preproc_images"
...
python trainer/preprocess.py \
  ...
  --output_path "${GCS_PATH}/preproc_images" \
  --job_name "${DFJOB_ID}e" \
  --num_workers 3 \
  --cloud
...
python trainer/preprocess.py \
  --input_dict "$DICT_FILE" \
  --input_path "gs://cloud-ml-data/img/flower_photos/train_images_path_set.csv" \
