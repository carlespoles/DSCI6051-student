...
ignored_unlabeled_image = Metrics.counter('main', 'ignored_unlabeled_image')
...
class Default(object):
  ...
  IMAGE_GRAPH_CHECKPOINT_URI = (
      'gs://wellio-kadaif-tasty-images-project-images/inception_v3.ckpt')
...
def configure_pipeline(p, opt):
  ...
  input_source = beam.io.ReadFromText(
      opt.input_path, strip_trailing_newlines=True)
  label_source = beam.io.ReadFromText(
      opt.input_dict, strip_trailing_newlines=True)
  labels = (p | 'Read dictionary' >> label_source)#beam.Read(label_source))
  _ = (p
       | 'Read input' >> input_source#beam.Read(input_source)
